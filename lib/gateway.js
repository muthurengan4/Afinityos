  ...
  async dispatch(segments, ctx, deps) {
    ...
    if (path === '/sso/launch' && method === 'POST') {
      ...
      if (typeof token !== 'string') {
        // Structured error from loginAndGetToken — surface it
        return NextResponse.json({
          error: `Login to ${cfg.baseUrl}${cfg.loginPath || c.loginPath || '/api/auth/login'} failed.`,
          diagnostics: token,
        }, { status: 401 });
      }
    ...
    // Detailed login-probe: hits credential login endpoint(s) and returns diagnostics
    const connectorTestLoginMatch = path.match(/^\/connectors\/([^/]+)\/test-login$/);
    if (connectorTestLoginMatch && method === 'POST') {
      const user = await deps.getAuthUser(req);
      if (!user) return Gateway.unauthorized();
      if (!['org_admin', 'super_admin'].includes(user.role)) return Gateway.forbidden();
      const c = connectorRegistry.get(connectorTestLoginMatch[1]);
      if (!c) return Gateway.notFound('Connector not found');
      const cfg = await c.getConfig(user.organizationId);
      if (!cfg.baseUrl) return NextResponse.json({ error: 'baseUrl not configured' }, { status: 400 });
      if (!cfg.serviceEmail || !cfg.servicePassword) {
        return NextResponse.json({ error: 'serviceEmail + servicePassword required' }, { status: 400 });
      }
      // 1) First try the configured path
      const primary = await c.loginAndGetToken(user.organizationId, cfg);
      if (typeof primary === 'string') {
        return Gateway.ok({
          ok: true, tokenPreview: `${primary.slice(0, 12)}…`, loginPath: cfg.loginPath || c.loginPath || '/api/auth/login',
        });
      }
      // 2) On failure — auto-probe alternate login paths so the user can pick one
      const candidates = ['/api/auth/login', '/auth/login', '/api/login', '/api/v1/auth/login', '/api/v1/login', '/login', '/token', '/api/token', '/api/user/login', '/api/users/login'];
      const alreadyTried = cfg.loginPath || c.loginPath || '/api/auth/login';

      // 2a) Also probe alternate BASE hostnames — many production stacks put the frontend
      // on app.<domain> and the API on api.<domain>. If the Base URL host starts with a
      // frontend-ish subdomain, add candidate hosts with common API-subdomain rewrites.
      const baseHosts = [cfg.baseUrl];
      try {
        const u = new URL(cfg.baseUrl);
        const parts = u.hostname.split('.');
        const rewrites = new Set();
        // app.<x> -> api.<x>, backend.<x>, server.<x>
        if (parts[0] === 'app' && parts.length >= 2) {
          for (const alt of ['api', 'backend', 'server']) {
            rewrites.add(`${u.protocol}//${alt}.${parts.slice(1).join('.')}`);
          }
        }
        // bare <domain> -> api.<domain>
        if (parts.length === 2) {
          rewrites.add(`${u.protocol}//api.${u.hostname}`);
        }
        // www.<x> -> api.<x>
        if (parts[0] === 'www' && parts.length >= 2) {
          rewrites.add(`${u.protocol}//api.${parts.slice(1).join('.')}`);
        }
        for (const r of rewrites) baseHosts.push(r);
      } catch { /* invalid URL, skip host rewrites */ }

      const probes = [];
      let winner = null;
      outer: for (const host of baseHosts) {
        for (const p of candidates) {
          if (host === cfg.baseUrl && p === alreadyTried) continue;
          try {
            const url = new URL(p, host).toString();
            ...
            try {
              ...
              tokenFound = j?.access_token || j?.accessToken || j?.token || j?.jwt || j?.data?.token || null;
            } catch { /* not JSON */ }
            probes.push({ host, path: p, status: r.status, ok: r.ok, tokenFound: !!tokenFound, bodySnippet: t.slice(0, 200) });
            if (tokenFound) {
              winner = { host, path: p };
              break outer;
            }
          } catch (e) {
            probes.push({ host, path: p, error: e.message });
          }
        }
      }
      return Gateway.ok({
        ok: false,
        primaryDiagnostics: primary,
        alternateProbes: probes,
        suggestedBaseUrl: winner && winner.host !== cfg.baseUrl ? winner.host : null,
        suggestedLoginPath: winner ? winner.path : null,
      });
    }
