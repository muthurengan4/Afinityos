  ...
  async dispatch(segments, ctx, deps) {
    ...
    if (connectorTestLoginMatch && method === 'POST') {
      ...
      for (const p of candidates) {
        if (p === alreadyTried) continue;
        try {
          const url = new URL(p, cfg.baseUrl).toString();
          ...
          let tokenFound = null;
          try { const j = JSON.parse(t); tokenFound = j?.access_token || j?.accessToken || j?.token || j?.jwt || null; } catch { /* not JSON */ }
          probes.push({ path: p, status: r.status, ok: r.ok, tokenFound: !!tokenFound, bodySnippet: t.slice(0, 200) });
          if (tokenFound) break;
        } catch (e) {
          probes.push({ path: p, error: e.message });
        }
      }
      const winner = probes.find((p) => p.tokenFound);
      return Gateway.ok({
        ok: false,
